...
typedef ShareDialogItemClickCallBack = void Function(
    int shareChannel, int customIndex);

/// 获取自定义分享item标题
/// index为自定义分享item索引
typedef ShareDialogGetCustomShareItemTitle = String? Function(int index);

/// 获取自定义分享item图标
/// index为自定义分享item索引
typedef ShareDialogGetCustomShareItemIcon = Widget? Function(int index);
...
class ShareDialog extends StatelessWidget {
  ...
  /// 点击事件
  final ShareDialogItemClickCallBack? clickCallBack;

  /// 回调获取名称
  final ShareDialogGetCustomShareItemTitle? getCustomChannelTitle;

  /// 回调获取图片(Widget)
  final ShareDialogGetCustomShareItemIcon? getCustomChannelWidget;
  ...
  Widget _configDialogShareItems() {
    ...
    for (int index = 0; index < shareChannels.length; index++) {
      title = null;
      image = null;
      if (shareChannels[index] == BaseShareItemConstants.shareCustom) {
        // 获取自定义channel信息
